...
n_Lax = 25;
x_Lax = linspace(lb(1),ub(1),n_Lax);
u_Lax = zeros(nsteps+1, n_Lax);
u_Lax(1,:) = Exact_solution(0,x_Lax);
dx = x_Lax(2)-x_Lax(1);
c = dt/dx;
aa = 0.5*c*(1.+c);
bb = (1.0-c^2);
...
for i = 1:nsteps
    ...
    u_Lax(i+1,1) = aa*u_Lax(i,n_Lax-1) + bb*u_Lax(i,1) + cc*u_Lax(i,2);
    u_Lax(i+1,end) = aa*u_Lax(i,n_Lax-1) + bb*u_Lax(i,end) + cc*u_Lax(i,2);
...
%         subplot(2,num_plots,k)
        ...
        plot(x_Lax, u_Lax(i+1,:), 'k-.','LineWidth',2);        
